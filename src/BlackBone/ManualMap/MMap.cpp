...
NTSTATUS MMap::ProbeRemoteSxS( std::wstring& path )
{
    ...
    {
        _UNICODE_STRING_T<uint32_t> OriginalName;
        ...
        status = _process.memory().Write(memPtr, OriginalName);
        ...
        status =  _process.memory().Write(memPtr + dll1Offset, DllName1);
        //status = fillStr(_UNICODE_STRING_T<uint32_t>());
    }
    else 
    {
        _UNICODE_STRING_T<uint64_t> OriginalName;
        std::remove_reference<decltype(OriginalName)>::type DllName1 = { 0 };
