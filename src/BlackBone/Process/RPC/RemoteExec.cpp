    ...
    )
{
    ...
    auto thread = _threads.CreateNew( _userCode.ptr() + size, _userData.ptr()/*, HideFromDebug*/ , CreateThreadFlags::NoThreadFlags);
    if (!thread)
...
{
    auto thread = _threads.CreateNew( pCode, arg/*, HideFromDebug*/, CreateThreadFlags::NoThreadFlags);
    if (!thread)
        return thread.status;
...
call_result_t<DWORD> RemoteExec::CreateWorkerThread()
{
    ...
    if(!_workerThread || !_workerThread->valid())
    {
        ...
        */
        ...
        auto thd = _threads.CreateNew( _workerCode.ptr() + sizeof( LARGE_INTEGER ), _userData.ptr()/*, HideFromDebug*/, CreateThreadFlags::NoThreadFlags);
        if (!thd)
            return thd.status;
