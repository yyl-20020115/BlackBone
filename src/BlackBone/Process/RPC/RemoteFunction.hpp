...
public:
    ...
    RemoteFunction( Process& proc, ptr_t ptr  )
        : RemoteFunctionBase<cc_cdecl, R, Args...>( proc, ptr )
    { 
    }
};
...
public:
    ...
    RemoteFunction( _Process& proc, R( __stdcall* ptr )(Args...) )
...
public:
    ...
    RemoteFunction( _Process& proc, R( __thiscall* ptr )(Args...) )
        : RemoteFunctionBase( proc, reinterpret_cast<ptr_t>(ptr) ) 
    { 
    }
};
...
public:
    ...
    RemoteFunction( _Process& proc, R( __fastcall* ptr )(Args...) )
        : RemoteFunctionBase( proc, reinterpret_cast<ptr_t>(ptr) ) 
    { 
    }
};
...
{
    return RemoteFunction<T>( process, (ptr_t)ptr );
}
...
{
    call_result_t<exportData> ptr = process.modules().GetExport( modName, name_ord );
    return RemoteFunction<T>( process, (ptr ? ptr->procAddress : (ptr_t)0) );
}
